...
The following steps will integrate Tailscale by using an OAuth2.0 API Client with the below API permissions. Currently setup is limited to using an API Client with a client id, client secret, and domain.
...
<ResponseField name="Tenant Domain" type="string" required>
  The primary domain for the tailnet.
</ResponseField>